...
library Permissions {
    ...
    function scopeFunction(
         ...
         * )
         */
        uint256 scopeConfig = FunctionConfig.pack(0, options, false, length);
        for (uint256 i = 0; i < length; i++) {
            scopeConfig = FunctionConfig.packParameterRaw(
                scopeConfig,
                i,
                isScoped[i],
                paramType[i],
    ...
    function scopeFunctionExecutionOptions(
        ...
        //set scopeConfig
        uint256 scopeConfig = FunctionConfig.packOptions(
            role.functions[key],
            options
        );
    ...
    function scopeParameter(
        ...
        // set scopeConfig
        bytes32 key = keyForFunctions(targetAddress, functionSig);
        uint256 scopeConfig = FunctionConfig.packParameter(
            role.functions[key],
            index,
            true, // isScoped
            paramType,
