...
describe("TypeTree", () => {
  ...
  describe("inspect", () => {
    ...
    describe("Logical Nodes (And / Or)", () => {
      ...
      describe("Variant", () => {
        it("keeps None encoding for variant And with heterogeneous children", async () => {
          const { inspect } = await loadFixture(setup);
          // And -> (Static, Dynamic) => None(Static, Dynamic)
          const result = await inspect({
            paramType: Encoding.None,
            operator: Operator.And,
            children: [
        ...
        it("keeps None encoding for variant Or with heterogeneous children", async () => {
          const { inspect } = await loadFixture(setup);
          // Or -> (Static, Tuple) => None(Static, Tuple)
          const result = await inspect({
            paramType: Encoding.None,
            operator: Operator.Or,
            children: [
        ...
        it("keeps None for structurally different children even if encodings match (deep diff)", async () => {
          const { inspect } = await loadFixture(setup);
          // Or -> (Tuple(Static), Tuple(Dynamic)) => None(...)
          const result = await inspect({
            paramType: Encoding.None,
            operator: Operator.Or,
            children: [
