...
describe("Roles", async () => {
  ...
  describe("Reentrancy", () => {
    it("blocks reentrant call - doNothing is never executed", async () => {
      ...
      // Setup: grant role to invoker (already enabled as module in setup)
      await roles.connect(owner).grantRole(invoker.address, roleKey, 0, 0, 0);

      // Setup: enable checker as module and grant same role
      await roles.connect(owner).enableModule(checkerAddress);
      await roles.connect(owner).grantRole(checkerAddress, roleKey, 0, 0, 0);

      await roles
        .connect(owner)
        .allowTarget(roleKey, checkerAddress, [], ExecutionOptions.None);
