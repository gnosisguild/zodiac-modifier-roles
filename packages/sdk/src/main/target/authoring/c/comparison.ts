...
import { AbiType, Operator } from "../../../types"
...
export const bitmask =
  ...
  (abiType: ParamType) => {
    ...
    if (paramType !== AbiType.Static && paramType !== AbiType.Dynamic) {
      throw new Error(
        `Bitmask can only be used for parameters with type Static or Dynamic, got: ${AbiType[paramType]}`
      )
    }
