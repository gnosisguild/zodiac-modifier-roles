...
export const allowRegularPool = (pool: Pool): PresetAllowEntry[] => {
  const poolFunctions: PresetAllowEntry[] = [
    ...
    { tokens: pool.tokens.filter(token => token.toLowerCase() !== '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'), spenders: [pool.address] },
    {
  ...
  if ("zap" in pool) {
    const zapFunctions = [
      ...
      { tokens: [...pool.zap.basePool.tokens], spenders: [pool.zap.address] },
      {
        ...
        signature: "add_liquidity(uint256[${pool.tokens.length+pool.zap.basePool.tokens.length-1}],uint256)",
      },
      {
        targetAddresses: [pool.zap.address],
        signature: "remove_liquidity_one_coin(uint256,int128,uint256)",
      ...
      {
        targetAddresses: [pool.zap.address],
        signature: "remove_liquidity(uint256,uint256[${pool.tokens.length+pool.zap.basePool.tokens.length-1}])",
      },
      {
        targetAddresses: [pool.zap.address],
        signature: "signature': 'remove_liquidity_imbalance(uint256[${pool.tokens.length+pool.zap.basePool.tokens.length-1}],uint256)",
      },
    ];
