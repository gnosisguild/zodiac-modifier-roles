...
const MULTICALL_SELECTOR = "0x17384664"
...
export const registerBundler3Unwrapper = (
  ...
  const bundler3Interface = new Interface([
    "function multicall((address to,bytes data,uint256 value,bool skipRevert,bytes32 callbackHash)[]) payable",
  ])

  const multicallSelector = bundler3Interface.getFunction("multicall")!
  ...
  const data = rolesContract.interface.encodeFunctionData(
    ...
    [bundler3Address, multicallSelector, MORPHO_BUNDLER3_UNWRAPPER]
  ) as `0x${string}`
