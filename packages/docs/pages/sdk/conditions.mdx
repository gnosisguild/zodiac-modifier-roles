...
For providing them as an object with named keys, it must be wrapped in an `c.eq({ name: 'foo', age: 30 })` condition function, to enforce an exact match rather than a pattern match.
...
be invoked with the expected value. `c.avatar` is a special case since it does
...
Allows defining a [matching pattern](#matching-patterns) to check the value in scope against.
...
This condition is most commonly used at the root node of the conditions tree for defining the conditions structure to be applied on the parameters of the call.
...
This makes it convenient to use typed allow kits for scoping bytes parameters.
