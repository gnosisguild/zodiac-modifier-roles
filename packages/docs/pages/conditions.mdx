...
The root condition node generally targets the entire calldata range after the 4 bytes function selector. It uses `ParamType.AbiEncoded` for defining how to decode the bytes in that range.
