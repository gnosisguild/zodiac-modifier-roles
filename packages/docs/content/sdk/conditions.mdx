...
An allowance is referenced by its key, which you can pass as the label you named
it with. The SDK encodes it to `bytes32`, and leaves a key that is already
encoded alone — so `encodeKey("my-allowance")` keeps working wherever a label
is accepted.
...
```typescript
c.withinAllowance("my-allowance");
```
...
```typescript
c.calldataMatches([], [], {
  etherWithinAllowance: "my-eth-allowance",
});
```
