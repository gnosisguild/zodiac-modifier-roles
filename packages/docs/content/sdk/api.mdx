...
already encoded is returned unchanged, so it is safe to apply more than once.
Throws for a key that does not fit into bytes32, and for a `0x`-prefixed value
that is not exactly 32 bytes — that is a malformed encoded key rather than a
label, and packing its characters would silently address something else.
