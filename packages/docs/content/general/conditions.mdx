...
Each node in the tree defines a condition [operator](#operator) and specifies an [encoding](#encoding) for the value in scope.
Evaluating the condition tree on transaction calldata starts at the root node.
As the tree is traversed the calldata is decoded and specific ranges are plucked according to the encoding definitions specified with the nodes.
...
Passes if the calldata is empty (i.e., the transaction has no calldata beyond the function selector, or no selector at all).
Useful for selector-only checks or for validating plain ether transfers.
...
A [price adapter](/general/price-adapters) can be used to normalize the values to a common denomination before comparison.
...
Matches trailing elements of an array against a condition pattern.
This is useful when the meaningful elements are at the end of an array and the length of the prefix is not known ahead of time.

### Extraction Operators

#### `Slice`

Extracts a range of bytes from the value currently in scope.
The extracted bytes are made available for subsequent conditions in the tree to reference.
This enables fine-grained inspection of packed or concatenated data.
...
[Allowances](/general/allowances) serve as a mechanism for tracking quotas.
...
Checks the value in scope against a [custom condition](#custom-conditions) defined in the `compValue` field.
...
The following condition enforces that the Balancer [single swap](https://docs.balancer.fi/integration-guides/swapping/swapping-custom-paths-with-router.html#single-swap) function must only be used to swap WETH for DAI and vice versa. Additionally, the sender and recipient for the swap must be set to the avatar address.
