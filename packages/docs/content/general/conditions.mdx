...
- Set conditions on elements of an array type value (via `Operator.ArrayEvery`)
...
Used with `ParamType.AbiEncoded` it allows setting conditions on specific parameters of the function call or on selected ABI encoded variables within a bytes value.
...
The root condition node generally targets the entire calldata range after the 4 bytes function selector. It uses a `ParamType.AbiEncoded` matches structure defining how to decode the bytes in that range.
